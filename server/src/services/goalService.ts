...
export async function createGoal(
  ...
  const { rows } = await pool.query<GoalRow>(
    `INSERT INTO goals (id, user_id, title, description, category, status, target_date)
     ...
     RETURNING id, user_id, title, description, category, status, target_date, created_at, updated_at`,
...
export async function updateGoal(
  ...
  const target_date =
    patch.target_date !== undefined ? patch.target_date : current.target_date;
